        ...
        function setVelocity(self, varargin)
            ...
            elseif length(varargin{1}) == 2
                ...
                if length(varargin) >= 2
                    ...
                    duration = duration + self.timeNoise(duration);
            ...
            if vel(1) < 0
                vel(1) = 5*ceil(vel(1)/5);
            elseif vel(1) > 1
                vel(1) = 5*floor(vel(1)/5);
            end
            if vel(2) < 0
                vel(2) = 5*ceil(vel(2)/5);
            elseif vel(2) > 1
                vel(2) = 5*floor(vel(2)/5);
            end
...
%             self.robotAccel = accel;
    ...
    methods(Static, Hidden, Access=protected)
        ...
        function noise_time = timeNoise(duration)
            noise_time = randn(1) * piBotSim.robotTimeNoise;
        end
